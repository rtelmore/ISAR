...
df <- dplyr::as_tibble(games$LeagueGameLog) |>
  janitor::clean_names()
saveRDS(df, "tmp/nba-games-end-2025.rds")
...
for(i in 1:600){
  ...
  try(results[[i]] <- nba_boxscorefourfactorsv2(game_ids[i]))
}

saveRDS(results, "tmp/four-factors-box-600-2025.rds")
...
for(i in 601:1200){
  ...
  try(results[[i-600]] <- nba_boxscorefourfactorsv2(game_ids[i]))
}

saveRDS(results, "tmp/four-factors-box-601-1200-2025.rds")
...
for(i in 1201:1230){
  ...
  try(results[[i-1200]] <- nba_boxscorefourfactorsv2(game_ids[i]))
}

saveRDS(results, "tmp/four-factors-box-1201-1230-2025.rds")

ff_list <- c(readRDS("tmp/four-factors-box-600-2025.rds"),
             readRDS("tmp/four-factors-box-601-1200-2025.rds"),
             readRDS("tmp/four-factors-box-1201-1230-2025.rds"))

# index <- which(sapply(ff_list, is.null))
# for(i in index) {
#   if(i %% 20 == 0){
#     cat(sprintf("Game id %s at %s \n", i, Sys.time()))
#   }
#   try(ff_list[[i]] <- nba_boxscorefourfactorsv2(game_ids[i]))
# }

saveRDS(ff_list, "tmp/ff-box-2025.rds")

df_tmp <- readRDS("tmp/ff-box-2025.rds")

df <- do.call(rbind,
              lapply(df_tmp, FUN = function(x) x$sqlTeamsFourFactors)) %>%
  janitor::clean_names()

saveRDS(df, "tmp/ff-team-2025.rds")
...
for(i in 1:600){
  ...
  try(results[[i]] <- nba_boxscoreadvancedv2(game_ids[i]))
}

saveRDS(results, "tmp/advanced-box-600-2025.rds")
...
saveRDS(results, "tmp/advanced-box-601-1200-2025.rds")
...
saveRDS(results, "tmp/advanced-box-1201-1230-2025.rds")

adv_list <- c(readRDS("tmp/advanced-box-600-2025.rds"),
              readRDS("tmp/advanced-box-601-1200-2025.rds"),
              readRDS("tmp/advanced-box-1201-1230-2025.rds"))

index <- which(sapply(adv_list, is.null))
for(i in index) {
  if(i %% 20 == 0){
    cat(sprintf("Game id %s at %s \n", i, Sys.time()))
  }
  try(adv_list[[i]] <- nba_boxscoreadvancedv2(game_ids[i]))
}

saveRDS(adv_list, "tmp/adv-box-2025.rds")


df_tmp <- readRDS("tmp/adv-box-2025.rds")

df <- do.call(rbind,
              lapply(df_tmp, FUN = function(x) x$TeamStats)) %>%
  janitor::clean_names()

saveRDS(df, "tmp/adv-team-2025.rds")
...
saveRDS(results, "tmp/misc-box-600-2025.rds")
...
saveRDS(results, "tmp/misc-box-601-1200-2025.rds")
...
saveRDS(results, "tmp/misc-box-1201-1230-2025.rds")

misc_list <- c(readRDS("tmp/misc-box-600-2025.rds"),
               readRDS("tmp/misc-box-601-1200-2025.rds"),
               readRDS("tmp/misc-box-1201-1230-2025.rds"))

index <- which(sapply(misc_list, is.null))
for(i in index) {
  if(i %% 20 == 0){
    cat(sprintf("Game id %s at %s \n", i, Sys.time()))
  }
  try(misc_list[[i]] <- nba_boxscoremiscv2(game_ids[i]))
}

saveRDS(misc_list, "tmp/misc-box-2025.rds")
...
saveRDS(df, "tmp/misc-team-2025.rds")
